...
## 二十一、严谨模型比较架构升级计划（YONOD benchmark pipeline）

> 本节根据 `project-docs/goal.md` 的 QA-R004 追加，吸收 Polaris Method Comparison 的公平比较规范与 Expansion ML Comparison 的实验编排思路。实施目标是升级 **YONOD 主线的反应产率 benchmark 能力**，而非复制 ADME 数据、标签变换或深度模型矩阵。
>
> 已确认的决策：反应指纹聚类与底物骨架/组分留出均作为可配置分组策略；默认验收为 5 次重复 × 5 折分组交叉验证，另设低成本 smoke 配置；统计结果同时覆盖模型维度和描述符维度的比较。

### 21.1 项目概述

#### 21.1.1 目标

将当前“描述符 × 模型 → 单次 KFold 汇总分数”的执行模式拆分为可审计的实验层：

```text
规范数据集 + benchmark 配置
        │
        ├─→ 分组器 → split manifest（唯一、可复用）
        │                         │
        │                         └─→ descriptor × model × repeat × fold 任务
        │                                                   │
        │                               状态库 ← 预测库 + 折级运行元数据
        │                                                   │
        └──────────────────────────────────→ 独立指标/统计模块 → 报告升级
...
核心原则是：同一组比较必须共享数据版本、特征定义与 split manifest；模型训练只产出预测和运行元数据；指标、统计检验与结论由独立模块从预测库重建。这样可避免不同模型“各自随机切分”而产生不公平的排行榜。

#### 21.1.2 规划输出与目录边界

后续 builder 应优先在现有 `yonod/` 包和每次运行的 `results/<run_id>/` 下实现，不移动现有数据集和入口。建议新增职责如下（具体文件名可在构建时按现有模块风格调整）：

```text
yonod/
├── splits/                 # 分组策略与 split manifest 读写
├── benchmark/              # 任务编排、状态恢复、运行元数据
├── metrics/                # 指标重建、置信区间与统计检验
└── models/
    └── lightgbm_model.py   # LightGBM 适配器

results/<run_id>/
├── manifests/
│   ├── run_manifest.json
│   └── split_manifest.parquet
├── state/tasks.parquet     # 或 SQLite；仅记录任务状态与错误摘要
├── predictions/            # 每个成功任务的样本级预测分片
├── metrics/                # 折级、组合级与统计汇总表
├── figures/                # 散点、稳定性、显著性与比较图
└── reports/                # HTML/Markdown 可追溯报告
```

`results/` 是可再生成的运行产物，默认不应作为一次实现提交的大文件。计划中的配置、代码和小型 smoke fixture 应与源码一同版本化；数据版本哈希、配置哈希和 Git 提交号写入 `run_manifest.json`。

### 21.2 可行性、依赖与风险概览

#### 21.2.1 可行性结论

该升级与现有架构兼容：`main.py` 已负责统一执行、`yonod/evaluate.py` 已有描述符/模型注册表、`yonod/universal/report.py` 已能生成 HTML/Markdown 报告。主要改造是把各模型内部的随机 `KFold` 循环迁移为由外部 manifest 驱动的统一 fold 执行器，属于可控的接口重构，而不是重写描述符管线。

需要明确的限制：

- “反应指纹相似性”没有唯一学术标准。第一版必须把输入列、反应指纹定义、相似度阈值/聚类算法、随机种子和 fallback 策略落入 manifest，不能把某一参数称为普适真值。
- 25 个折 × 每个描述符/模型组合会显著拉长运行时间；因此先建立 2×2 或 1×3 的 smoke 配置，再运行默认 5×5。
- Tukey HSD 适合比较折级聚合指标的多组均值，但其独立性/正态性假设在交叉验证中并不完美。报告必须同时给出原始折级分布、配对差值、效应量和方法假设，避免将 p 值解释为绝对结论。

#### 21.2.2 新增依赖

在现有环境基础上，实施时最低要求如下：

- `lightgbm >= 4.3`：新增树模型回归基线。
- `scipy >= 1.11`：置信区间、配对检验等统计计算。
- `statsmodels >= 0.14`：Tukey HSD 多重比较；若不能引入，应明确选择的等价实现和差异。
- `pyarrow >= 15.0`：预测库和 manifest 的 Parquet 读写。
- `scikit-learn >= 1.4`、`pandas >= 2.0`、`numpy >= 1.26`：分组切分、数据结构和指标计算。

官方源优先；仅在失败或超时后使用镜像：

```powershell
# 1. 官方源
pip install "lightgbm>=4.3" "scipy>=1.11" "statsmodels>=0.14" "pyarrow>=15.0"

# 2. 若官方源失败或超时，临时使用清华镜像
pip install "lightgbm>=4.3" "scipy>=1.11" "statsmodels>=0.14" "pyarrow>=15.0" -i https://pypi.tuna.tsinghua.edu.cn/simple --trusted-host pypi.tuna.tsinghua.edu.cn
```
...
| 顺序 | 阶段 | 前置依赖 | 阻塞原因 |
|---:|---|---|---|
| 21.1 | 配置与 run 标识 | 规范数据集、稳定 `sample_id` | 无稳定样本标识就无法复用 split 或审计预测 |
| 21.2 | 反应分组与 split manifest | 21.1、可配置 SMILES 列角色 | 后续所有方案必须共享切分 |
| 21.3 | fold 执行契约与预测库 | 21.2、现有特征/模型适配器 | 指标和恢复机制以其输出为数据源 |
| 21.4 | 任务状态与可恢复编排 | 21.3 | 需要能判定一个任务是否完整落盘 |
| 21.5 | LightGBM 基线 | 21.3 | 必须接入统一执行器，不能另走独立 CV |
| 21.6 | 独立指标与统计 | 21.3、21.5 | 依赖完整、结构统一的预测库 |
| 21.7 | 报告升级 | 21.6、运行元数据 | 图表与结论必须引用可追溯数据 |
| 21.8 | smoke/默认验收与回归测试 | 21.1–21.7 | 验证完整闭环及无泄漏性质 |

### 21.3 开发计划

### 步骤21.1：定义 benchmark 配置、运行标识与数据版本契约

#### 目标说明

先固定一次实验“究竟比较什么”。配置应将数据、列角色、描述符、模型、分组策略、CV 参数、随机种子、输出位置和统计指标集中声明；执行时为它生成不可混淆的 `run_id` 与 `config_hash`。这是避免长实验中不同参数结果被误合并的基础。

#### 具体操作

后续 builder 应定义版本化配置（YAML 或 JSON 均可），至少包含：

```yaml
benchmark:
  dataset_path: dataset/...
  dataset_sha256: "<由运行时计算>"
  sample_id_col: row_id
  label_col: yield
  descriptors: [morgan, rdkit2d]
  models: [rf, xgb, lightgbm]
  grouping:
    strategy: reaction_fingerprint_cluster  # 或 substrate_scaffold / component_holdout
  cv:
    n_repeats: 5
    n_splits: 5
    seed: 42
...
运行前校验 `sample_id` 非空且唯一、标签可解析、指定 SMILES 列存在；写入 `run_manifest.json` 时记录配置哈希、数据 SHA-256、代码提交号、Python/依赖版本、主机信息和启动时间。任何影响训练集/验证集或特征的配置变化都必须生成新的 `run_id`，不得覆盖原结果。

#### 验收标准

- 缺少 `sample_id`、标签列、SMILES 列或重复样本标识时，任务在训练前失败并给出明确错误。
- 同一配置和同一数据重复创建时得到相同 `config_hash`；改变分组参数、模型参数或数据内容时哈希改变。
- 每个运行目录均有可解析的 `run_manifest.json`，并能追溯数据版本、配置和代码版本。

#### 依赖

现有 CSV 加载/列角色配置逻辑；`pyyaml >= 6.0`、`pandas >= 2.0`、`python >= 3.9`。

#### 风险与验证策略

风险是把源 CSV 的行号误当成长期样本标识，导致数据排序后 manifest 失效。验证时对同一 CSV 打乱行顺序后，以 `sample_id` 连接，确认样本集合与配置哈希语义正确；对人为重复 ID 的 fixture 断言执行失败。

---

### 步骤21.2：实现可配置反应分组与可复用 split manifest

#### 目标说明

为所有候选方案先生成一份唯一的分组重复交叉验证切分计划，消除每个模型独立随机 KFold 的不公平性，并用更困难的反应相似性分组检验泛化能力。

#### 具体操作

第一版需支持以下策略，并统一输出 `group_id`：

1. `reaction_fingerprint_cluster`：依据已声明的反应物/产物/条件列生成反应级指纹，按配置的相似度阈值或聚类算法形成组。
2. `substrate_scaffold`：以指定底物列的 Bemis–Murcko scaffold 为组；无有效 scaffold 时使用明确、可记录的 fallback（例如规范 SMILES 或单独 `invalid_scaffold` 组）。
3. `component_holdout`：以一个或多个指定反应组分的规范化 SMILES/组合键作为组，用于模拟未见底物或条件。

切分生成器接收 `n_repeats`、`n_splits`、`seed`，默认输出 5×5 分组 CV。每条 manifest 记录至少包含：
...
其中 `role ∈ {train, valid}`。同一 `split_id` 的全部模型和描述符只能读取，不得重新生成。若分组数量少于 `n_splits` 或单个组过大导致无法合理分折，必须停止并报告组数、最大组大小与可选降级配置，绝不静默回退到普通 KFold。

#### 验收标准

- 默认配置产生 5 个 repeat × 5 个 fold，且每个 `sample_id` 在每个 repeat 恰出现一次 valid、其余折为 train。
- 对每个 `(repeat, fold)`，训练集与验证集 `group_id` 交集为空。
- 所有描述符和模型读取同一 `split_id`；运行日志能证明没有各自重新切分。
- 相同数据与参数重新生成的 manifest 字节内容或规范化哈希一致。

#### 依赖

步骤21.1 的稳定样本标识、RDKit 描述符能力、`scikit-learn >= 1.4`、`pyarrow >= 15.0`。反应指纹策略还依赖已明确的列角色与规范化规则。

#### 风险与验证策略

高频风险包括 fingerprint 聚类参数导致一个“超级大组”、scaffold 解析失败和无效 SMILES。验证需使用合成小数据集：一组重复反应、两组相似底物、一个无效结构；断言无组泄漏、fallback 被记录、组数不足时失败信息可操作。真实数据先输出 group-size 分布图和有效折样本数，再批准默认 5×5 运行。

---

### 步骤21.3：建立外部 fold 执行契约与逐折预测库

#### 目标说明

将模型适配器从“内部自行创建 KFold”转为“接受外部 train/valid 索引或 manifest 指定切分”。每一个成功任务都保存样本级预测，后续指标、统计和报告无需重新训练。
...
```text
(run_id, split_id, descriptor, model, repeat, fold)
```

执行器只在该 fold 的训练样本上 fit 标准化、缺失值填补、PCA、特征选择和模型；验证样本仅 transform/predict。每个成功单元写一份不可覆盖的预测分片（优先 Parquet），字段至少为：

```text
run_id, config_hash, split_id, sample_id, group_id,
descriptor, model, repeat, fold, y_true, y_pred,
feature_schema_hash, train_time_s, predict_time_s
```

同时写折级元数据：训练/验证样本数、特征维度、随机种子、开始/结束时间、软件版本、可选峰值内存与错误状态。写入采用“临时文件 → 校验行数/模式 → 原子重命名”流程，防止中断生成半份可误判为成功的文件。

#### 验收标准

- 任何成功预测分片行数等于该 fold 验证集样本数，且 `sample_id` 与 manifest 的 valid 行一一对应。
- 每个 `(descriptor, model, repeat, fold, sample_id)` 唯一；重复运行不会无提示产生重复预测。
- 通过在训练特征中插入异常值、验证特征保持正常的测试，证明 scaler/PCA 等只在训练折 fit。
- 折级元数据可独立关联到 manifest、配置和预测分片。

#### 依赖

步骤21.2 的 manifest、现有 `yonod/evaluate.py` 注册表、描述符特征构建逻辑及各模型适配器；`pandas >= 2.0`、`pyarrow >= 15.0`。

#### 风险与验证策略

当前部分模型将 KFold 封装在各自 `cross_validate` 内部，直接复用会绕过 manifest。实施时应先为所有现有模型定义单折 `fit/predict` 适配层，再保留旧 `cross_validate` 接口作兼容路径。验证以 RF、XGBoost、SVM 各跑一折 smoke，并对 AutoGluon 的内部 holdout 行为专门记录：若其不能保证外部 fold 内公平训练，须在默认严格矩阵中禁用或提供已验证的 fold 内配置，而非默默混入结果。

---

### 步骤21.4：实现可恢复任务编排与状态库

#### 目标说明

将长实验拆为细粒度、可定位、可安全重跑的任务，避免一次中断使整个 5×5 矩阵报废，也避免盲目重复训练并覆盖已完成结果。

#### 具体操作

任务状态至少为 `pending`、`running`、`succeeded`、`failed`、`skipped`。状态记录字段包括任务唯一键、尝试次数、开始/结束时间、输入配置哈希、输出文件哈希、错误摘要和 worker/主机信息。重启策略为：

- 状态为 `succeeded` 且输出文件校验通过、`config_hash`/`split_id` 一致：默认跳过。
- `failed` 或输出不完整：保留历史失败原因，允许 `--rerun-failed` 或按任务键选择性重跑。
- 配置或 manifest 不一致：创建新 `run_id`，禁止复用旧成功状态。
- 遗留 `running`：启动时检查心跳与输出完整性，标为 `interrupted`/`failed`，不得误认为成功。

初版可选 Parquet + 文件锁或 SQLite。若任务可能并行，优先 SQLite 事务或一个单写入协调器；不要让多个 worker 并发追加同一个 CSV。

#### 验收标准

- 人为在第一个模型的一半 fold 后中断，再启动时只执行缺失或失败任务。
- 删除一个成功预测分片或篡改其哈希后，状态检查能发现并重新排队，而不是跳过。
- 失败任务保留异常类型、摘要和最近一次日志路径；成功任务保留可验证输出路径。
- 完成任务数、失败任务数、待运行任务数与 manifest 的理论任务数一致。

#### 依赖

步骤21.3 的原子预测落盘、`run_id`/`config_hash`、可写结果目录；并行执行时还依赖所选状态存储的锁/事务机制。

#### 风险与验证策略

Windows 上文件锁和多进程写入较容易冲突。第一版先以单进程、串行任务正确性为验收底线，再扩展并行；测试应模拟键盘中断、异常、损坏文件与重复启动四种情形。状态库只存元数据，不存大数组，避免并发写放大和恢复变慢。

---

### 步骤21.5：接入 LightGBM 回归基线并纳入公平比较矩阵

#### 目标说明

LightGBM 是成本较低、对表格型拼接特征有竞争力的树模型基线。它的价值在于作为现有 RF、XGBoost、SVM、AutoGluon 的补充，而不是为了追逐单项最高分另起评估流程。

#### 具体操作

后续 builder 在 `yonod/models/` 增加与现有模型相同风格的 LightGBM 适配器，并注册到统一模型注册表。建议初始配置固定 `random_state`，显式设定 `n_estimators`、学习率、叶节点数、线程数和早停策略；调参必须只在训练折内部完成，或先以固定参数 baseline 运行。模型应接收外部 train/valid 索引，输出标准预测库字段和运行元数据。
...
lightgbm:
  ...
  num_leaves: 31
  random_state: 42
  n_jobs: 1  # smoke/可重复性优先；正式运行再按机器调整
```

#### 验收标准

- LightGBM 能在 smoke manifest 的每个 fold 成功训练并写出预测分片。
- LightGBM、RF、XGBoost 至少三种模型读取相同 `split_id`，每折验证样本集合一致。
- LightGBM 在缺失值、常量特征、样本量很小或特征稀疏时的行为有明确错误/警告与测试覆盖。
- 它的折级指标、运行时间和统计结果出现在同一矩阵中，不产生专用的不可比较汇总表。

#### 依赖

步骤21.3 的统一单折执行器、`lightgbm >= 4.3`、模型注册表与配置加载模块。

#### 风险与验证策略

LightGBM 对版本、线程数和早停参数较敏感；GPU 模式也会降低部分平台的复现一致性。第一版默认 CPU 且固定随机种子，报告写入 LightGBM 版本和参数。验证使用固定 fixture 对同版本结果的预测/指标容差进行断言，而非要求跨平台逐位一致。

---

### 步骤21.6：构建独立指标、置信区间与统计比较模块

#### 目标说明

让“训练”与“结论”解耦。该模块只读取 manifest、成功预测分片和折级元数据，重建 R²、RMSE、MAE、分布、置信区间、配对比较和 Tukey HSD；它既可在训练完成后运行，也可在新增组合后增量重建。

#### 具体操作

指标层需输出三类结构化表：

1. 折级指标：每个 `(descriptor, model, repeat, fold)` 的 R²、RMSE、MAE、有效样本数和时间。
2. 组合级汇总：均值、中位数、标准差、分位数，以及以“折”为统计单位的 bootstrap 95% CI；bootstrap 重采样次数、随机种子写入配置。
3. 统计比较：
   - **模型维度**：固定描述符、指标和 split，对模型的匹配折指标做配对比较，并生成 Tukey HSD 多重比较结果。
   - **描述符维度**：固定模型、指标和 split，对描述符的匹配折指标做同样比较。

每条统计结论要包含比较对象、指标、方向（例如 RMSE 越低越好）、样本折数、效应量/差值、原始 p 值、校正后 p 值、置信区间和标签：`significantly_better`、`no_significant_difference` 或 `significantly_worse`。若某个组合缺 fold，则默认不与完整组合混合比较，报告列出缺失原因。

#### 验收标准

- 从已保存的预测库重新计算结果，与训练时临时显示的折级指标在容差内一致。
- 人造“模型 A 每折都明显优于 B”的 fixture 可得到方向正确、显著的结论；完全相同预测的 fixture 得到无显著差异。
- 报告同时产生“固定描述符比较模型”和“固定模型比较描述符”的结果表，不允许仅生成一个总排名。
- 常量标签、单样本验证折、NaN/inf 预测、缺失 fold 均产生明确、可追溯的异常或排除记录。

#### 依赖

步骤21.3 的完整预测库、步骤21.5 的 LightGBM 输出、`scipy >= 1.11`、`statsmodels >= 0.14`、`numpy >= 1.26`、`pandas >= 2.0`。

#### 风险与验证策略

交叉验证折并非完全独立，因此 p 值不应作为唯一证据。验证要求报告同时呈现折级散点/箱线图、配对差值和 CI；在方法章节注明比较单位、检验假设和局限。若正态性/方差假设不满足，应在配置中允许非参数配对检验作为敏感性分析，但不可将多种检验任选结果后只展示最有利的一种。

---

### 步骤21.7：升级 HTML/Markdown benchmark 报告

#### 目标说明

把当前“汇总指标 + 散点图”扩展为能够支持答辩、论文写作和结果追溯的 benchmark 报告，清晰回答：比较是否公平、模型是否稳定、差异是否显著、成本是否值得。

#### 具体操作

在现有 `yonod/universal/report.py` 的报告能力基础上，新增或扩展以下章节：

- 实验可追溯性：`run_id`、数据哈希、配置哈希、Git 提交、依赖版本、split 策略与参数。
- 切分审计：每 fold 的 train/valid 样本数、组数、最大组大小、组泄漏检查结果。
- 性能矩阵：描述符 × 模型的 R²/RMSE/MAE、95% CI、完成/失败 fold 数、训练时间。
- 预测与稳定性：OOF/逐折散点图、残差分布、跨 repeat/fold 箱线图或森林图。
- 统计比较：模型维度与描述符维度的比较表、Tukey HSD 结果、显著性/无显著差异/较差的可读结论。
- 成本与限制：特征时间、训练时间、磁盘占用、失败任务以及统计假设/数据局限。

报告的推荐结论格式为“在指定 split、指标和置信水平下，组合 A 相比组合 B 的差异为 …；统计标签为 …；不把无显著差异误写成性能完全相同”。

#### 验收标准

- 任一完整 `run_id` 可由命令单独重建 HTML 和 Markdown 报告，无需重新训练。
- 每个报告表和图都标出输入文件/`run_id`，并能追溯到预测库或统计表。
- 同时呈现两种比较维度、失败/缺失任务和统计方法局限；没有把单一最高均值直接标为“最佳模型”的无条件结论。
- 对 smoke 结果生成的报告包含所有章节；数据不足时显示“不可比较/原因”，而不是空白或误导性排名。

#### 依赖

步骤21.2 的 split 审计、步骤21.4 的状态库、步骤21.6 的指标/统计表、现有报告模块及 `matplotlib >= 3.8`。

#### 风险与验证策略

风险是报告依赖训练时内存对象，导致稍后无法重建。验证必须在新的 Python 进程中只提供 `run_id` 和结果目录生成报告；同时在小屏/长表场景检查 HTML 是否清楚标记省略或分页，而不是静默裁切统计结果。

---

### 步骤21.8：建立 smoke 配置、默认 5×5 验收与回归测试门

#### 目标说明

以两级运行保障开发速度与研究可信度：smoke 用很小配置验证接口、数据结构和恢复逻辑；默认 5×5 分组重复 CV 用于正式结论。任何新模型/描述符必须先通过 smoke 才能进入长实验。
...
cv: {n_repeats: 2, n_splits: 2, seed: 42}
descriptors: [morgan]
models: [rf, xgb, lightgbm]
max_rows: 100

# benchmark_default_5x5.yaml
cv: {n_repeats: 5, n_splits: 5, seed: 42}
grouping: {strategy: reaction_fingerprint_cluster, ...}
descriptors: [morgan, rdkit2d, ...]
models: [rf, xgb, svm, autogluon, lightgbm]
```

自动化测试至少覆盖：配置/schema 校验、分组无泄漏、manifest 可重复、fold 内预处理、预测分片 schema、状态恢复、LightGBM 接入、指标重建、模型/描述符双维统计、报告重建。默认 5×5 开始前输出任务总数、预估时间/磁盘空间，并要求结果目录为空或新的 `run_id`，防止误覆盖。

#### 验收标准

- smoke 在开发机可在合理时间内完成，且完整产出 manifest、状态、预测、指标、统计和 HTML/Markdown 报告。
- 默认配置理论任务数为 `描述符数 × 模型数 × 5 × 5`，状态库与该数量一致；每个成功组合有 25 个可用 fold 或记录完整失败原因。
- 回归测试在不依赖大型真实数据、GPU、外网或已存在结果目录的条件下运行。
- 发布正式结果前，通过 manifest 无泄漏、预测完整性、统计双维覆盖、报告可重建四项门禁。

#### 依赖

步骤21.1–21.7 的完整闭环、`pytest >= 8.0`（建议）、小型匿名/合成测试数据 fixture。

#### 风险与验证策略

smoke 数据如果只含很少分组，可能无法覆盖 2 折以上；fixture 必须设计为至少每策略拥有足够的独立 group。正式 5×5 的运行成本会随描述符数和 AutoGluon 成倍增长，先在 subset 估算单任务耗时和预测存储量；若资源不足，可减少候选组合或禁用高成本模型，但不得把 smoke 成绩写成正式结论。

### 21.4 阶段级验收与决策门

| 阶段 | 可交付物 | 必须通过的门槛 |
|---|---|---|
| A：公平切分闭环 | 配置、group_id、split manifest、无泄漏测试 | 所有方案使用同一 manifest；同 fold 不共享 group |
| B：可审计执行闭环 | 预测库、状态库、恢复演示 | 中断后只重跑不完整任务；预测与 manifest 一一对应 |
| C：模型矩阵闭环 | RF/XGB/LightGBM smoke 结果 | 三个模型统一外部 fold；LightGBM 不走独立 CV |
| D：统计与报告闭环 | 指标/CI/双维统计表、HTML/Markdown | 两类比较齐全，报告能在新进程从 `run_id` 重建 |
| E：正式研究运行 | 默认 5×5 分组重复 CV 结果包 | 通过四项门禁，失败任务与限制均在报告中披露 |

在 A–D 未完成前，不启动大规模描述符 × 模型完整矩阵；在 E 之前，不把任何单次 KFold 的排行榜作为项目的正式泛化结论。

### 21.5 Q&A 记录

### 通用问题

**Q：为什么不直接把现有每个模型的 `cross_validate` 改成 5×5？**
**A：** 因为若每个模型在内部自行随机切分，5×5 仍无法保证方案之间使用同一验证样本。先固化一个 split manifest，再由所有模型读取，才使差异可归因于模型或描述符，而不是偶然切分。

**Q：反应指纹分组与底物骨架/组分留出，哪一个是默认“正确答案”？**
**A：** 两者都不是普适唯一答案，因此都要可配置并写入 manifest。反应指纹聚类模拟整体反应相近性；骨架/组分留出更直接回答未见底物或条件的泛化。报告应说明采用的策略和它所代表的泛化问题。

**Q：Tukey HSD 显示无显著差异，是否说明两种方法完全一样？**
**A：** 不说明完全相同，只说明在当前数据、切分、指标与统计功效下没有检测到足够证据支持差异。报告还应给出差值、CI、折级分布和样本折数。

**Q：为什么将 LightGBM 列为基线，而不先接入更大的图模型？**
**A：** LightGBM 适配现有反应级拼接特征，成本低且可快速验证公平比较流程。多组分反应的图模型编码和算力要求更高，应在严谨基线稳定后，另行规划反应级图表示和资源评估。

### 21.6 下一步行动建议

1. 由 **project-builder-cn** 先实现步骤21.1–21.3 的配置、分组 manifest 和单折预测契约，并以 RF 跑通最小闭环。
2. 随后实现步骤21.4 的状态恢复与步骤21.5 的 LightGBM；只在 smoke 配置验证通过后扩大模型矩阵。
3. 最后完成步骤21.6–21.8 的统计、报告和正式 5×5 验收；生成的结论才可用于答辩或论文。
4. 若实施中发现分组策略、统计假设或结果目录设计需要架构调整，应回到 **project-planner-cn** 增量更新本节，而非临时改变研究结论口径。
