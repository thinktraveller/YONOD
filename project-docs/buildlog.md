...
### 执行的任务
- 新增以 SQLite 事务为基础的单进程任务状态库，覆盖 `pending`、`running`、`succeeded`、`failed`、`interrupted`、`skipped` 状态及完整任务唯一键。
- 实现原子任务认领、尝试次数、主机信息、心跳、错误类型/摘要、日志路径和状态事件历史；初版明确限定为单进程协调器，不允许多个 worker 直接并发写入状态库。
- 成功任务保存预测分片与折级元数据的 SHA-256，并校验 `run_id`、`config_hash`、`split_id`、描述符、模型、repeat、fold 全部一致。
- 重启审计会将文件缺失、哈希被篡改或元数据不匹配的成功任务重新排队；遗留 `running` 任务在心跳超时后标记为 `interrupted`；失败/中断任务仅在显式 `rerun_failed` 时重新认领。

### 关键变更
- 新增 `yonod/benchmark/task_state.py`：任务规格、SQLite 状态表/事件表、输出哈希记录与验证、心跳恢复、选择性重跑和汇总接口。
- 更新 `yonod/benchmark/__init__.py`：公开任务状态库、任务规格与输出审计接口，供后续编排器调用。

### 验证结果
- `python -m py_compile yonod/benchmark/task_state.py yonod/benchmark/__init__.py`：通过。
- 临时验证覆盖成功输出复用、预测分片哈希篡改后重新排队、失败任务默认不重跑/显式重跑、心跳超时标为 `interrupted`、错误摘要与尝试次数保留；输出 `PASS`。
- 临时验证脚本 `_verify/step21_4_task_state.py` 已在验证后删除，未纳入提交。

### 遇到的问题及解决方案
- 问题：Windows 下 SQLite 连接未显式关闭时，临时验证目录无法删除数据库文件。
- 解决：将初始化、汇总与查询连接改为显式关闭；复验通过，临时目录可正常清理。

### 下一步计划
- 步骤 21.5：接入 LightGBM 回归基线，并只通过既有外部 fold 执行器写入同一预测库。
